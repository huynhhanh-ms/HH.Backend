...
namespace PI.Application.Service
{
    public class WeighingHistoryService : BaseService, IWeighingHistoryService
    {
        ...
        public WeighingHistoryService(IUnitOfWork unitOfWork, ICurrentAccount currentAccount) : base(unitOfWork)
        {
        ...
        public async Task<ApiResponse<string>> Create(WeighingHistory request)
        {
            try
            {
                await _unitOfWork.BeginTransactionAsync();
                ...
                weighingHistory.CreatedDate = DateTime.Now;
                weighingHistory.UpdatedDate = DateTime.Now;
                weighingHistory.CreatedBy = _currentAccount.GetAccountId();
                weighingHistory.UpdatedBy = _currentAccount.GetAccountId();

                await _unitOfWork.Resolve<WeighingHistory>().CreateAsync(weighingHistory);
                await _unitOfWork.SaveChangesAsync();

                //save image
                //await SaveImage(request.ImageUrls, weighingHistory.ProductId);

                //save product unit
                //await SaveProductUnit(request.Units, weighingHistory.ProductId, weighingHistory.Name, request.MasterUnitId);
                await _unitOfWork.SaveChangesAsync();
                await _unitOfWork.CommitTransactionAsync();
                return Success<string>("Create product success");
            }
        ...
        public Task<PagingApiResponse<ProductResponse>> Search(WeighingHistory request)
        {
